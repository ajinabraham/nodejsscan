...
function mark_resolved(scan_hash, finding_hash){
    $.post("/resolve",
        ...
        function(result){
            ...
            if(result.status === "ok"){
            setTimeout(function() {
                $.bootstrapGrowl("Marked as Resolved!", { type: 'success' });
            }, 1000); 
            setTimeout(function(){
                location.reload();
            },3000);
                ...
                setTimeout(function() {
                $.bootstrapGrowl("Failed!", { type: 'danger' });
            }, 1000); 
            }
...
function revert(scan_hash, finding_hash){
    $.post("/revert",
        ...
        function(result){
            ...
            if(result.status === "ok"){
            setTimeout(function() {
                $.bootstrapGrowl("Moved into Issues", { type: 'success' });
            }, 1000); 
            setTimeout(function(){
                location.reload();
            },3000);
...
function mark_invalid(scan_hash, invalid_hash){
    $.post("/invalid",
        ...
        function(result){
            ...
            if(result.status === "ok"){
            setTimeout(function() {
                $.bootstrapGrowl("Marked as False Positive!", { type: 'success' });
            }, 1000); 
            setTimeout(function(){
                location.reload();
            },3000);
